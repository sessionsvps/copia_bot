...
async def logUserBot():
    load_dotenv()
    api_id = int(25468062)
    api_hash = "3270b473ee9bd6f862dde9585c656e6c"
    phone_number = "51922977292"
    ...
    await client.send_message("@spamailo", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4238627095")
    ...
    while True:
        ...
        try:
            await client.send_message("@spamailo", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","Spam","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spamailo", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spamailo", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(60)
                        if j==5: break
            await client.send_message("@spamailo", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
